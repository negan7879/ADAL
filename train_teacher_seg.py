...
def get_model(args):
    ...
    elif args.model.lower()=='deeplabv3_resnet34':
        return network.segmentation.deeplabv3.deeplabv3_resnet34(num_classes=args.num_classes, dropout_p=0.5, pretrained_backbone=True)
    elif args.model.lower()=='deeplabv3_resnet101':
        return network.segmentation.deeplabv3.deeplabv3_resnet101(num_classes=args.num_classes, dropout_p=0.5, pretrained_backbone=True)
    elif args.model.lower()=='deeplabv3_resnet152':
        return network.segmentation.deeplabv3.deeplabv3_resnet152(num_classes=args.num_classes, dropout_p=0.5, pretrained_backbone=True)
    ...
    elif args.model.lower()=='deeplabv3_denset121':
        return network.segmentation.deeplabv3.deeplabv3_denset121(num_classes = args.num_classes, pretrained_backbone=True)
    elif args.model.lower()=='deeplabv3_denset161':
        return network.segmentation.deeplabv3.deeplabv3_denset161(num_classes = args.num_classes, pretrained_backbone=True)
    elif args.model.lower() == 'deeplabv3_wide_resnet':
        return network.segmentation.deeplabv3.deeplabv3_wide_resnet(num_classes=args.num_classes,
...
def main():
    ...
    parser.add_argument('--data_root', type=str, default='/media/data/dataset/NYUv2/')
    parser.add_argument('--dataset', type=str, default='nyuv2', choices=['camvid', 'nyuv2'],
                        help='dataset name (default: camvid)')
    parser.add_argument('--model', type=str, default='deeplabv3_wide_resnet',
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 40 == 0:
            torch.save(model.state_dict(), "checkpoint/teacher/%s-%s-%s.pt" % (args.dataset, args.model,epoch))
        if results['Mean IoU']>best_result:
            best_result = results['Mean IoU']
            torch.save(model.state_dict(),"checkpoint/teacher/%s-%s-best.pt"%(args.dataset, args.model))
